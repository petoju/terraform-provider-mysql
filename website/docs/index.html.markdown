...
- `use_proxy` - (Optional) Whether to use the proxy defined in environment variables (*_PROXY). Defaults to `true`. Set to `false` to ignore any proxy environment variables.
